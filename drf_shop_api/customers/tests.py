...
class TestCustomerBonusWallet(BaseAPITest):
    def setUp(self):
        ...
        user_list = mixer.cycle(self.wallet_counts).blend(
    ...
    def test_list_as_owner(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(
            all(key in resp.data for key in list(BaseCustomerBonusWalletSerializer.Meta.fields)),
        )
    ...
    def test_sort_by_amount(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(
            is_increasing([float(item["amount"]) for item in resp.data["results"]]),
        )
...
class TestCustomerWishList(BaseAPITest):
    ...
    def test_filter_by_user(self):
        ...
        self.assertEqual(resp.data["user"], self.target.user.id)
        self.assertTrue(
            all(key in resp.data for key in list(BaseCustomerWishListSerializer().Meta.fields)),
        )
...
class TestCustomerComparisonList(BaseAPITest):
    def setUp(self):
        self.admin = self.create_and_login(
            email="admin@example.com",
            is_staff=True,
            is_superuser=True,
        )

...
class TestProduct(BaseAPITest):
    def setUp(self):
        ...
        self.screen_size = mixer.blend(
            Property,
            title="Screen size",
            unit=Units.INCH.name,
        )
        ...
        for number, prop, category in zip(
            range(self.product_count),
            properties,
            categories,
        ):
    ...
    def test_detail_product(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(
            all(key in resp.data for key in list(ProductSerializer.Meta.fields)),
        )
...
class TestProductComment(BaseAPITest):
    ...
    def test_update_own_comment(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(
            ProductComment.objects.get(id=self.target.id).content,
            "New content",
        )
...
class TestProperties(BaseAPITest):
    def setUp(self):
        ...
        for _ in range(self.prop_count):
            mixer.blend(
                Property,
                title=mixer.RANDOM,
                unit=random.choice(list(Units.__members__)),
            )
    ...
    def test_detail_property(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(
            all(key in resp.data for key in list(PropertySerializer.Meta.fields)),
        )
